...
- Квадратные скобки заменить на ковычки " так получится строка от массива.
...
| **findIndex** | `arr.findIndex(callback)` | Индекс или `-1` | Возвращает индекс первого элемента, удовлетворяющего условию. | Да (автомат) |
